...
GAME_update :: proc(game: ^Game) {
    ...
    GAME_update_exp(&game.exp_points, game.player.position)

    SHIP_update_player(&game.player, game.cursor_position, &game.ally_bullets)
    ...
    // update bullets
    GAME_update_bullets(&game.ally_bullets)
    GAME_update_bullets(&game.enemy_bullets)
...
GAME_update_bullets :: proc(blist: ^[dynamic]SHIP_Bullet) {
    i := 0
    for i < len(blist) {
        b := &blist[i]
        kill := SHIP_update_bullet(b)
